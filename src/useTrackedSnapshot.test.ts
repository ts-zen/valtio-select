...
describe("useTrackedSnapshot - Behavior with Returned Objects", () => {
  /**
   * Tests that a component re-renders when a property of the object
   * returned by the selector is mutated.
   */
  test("should re-render when a property of the returned object changes", async () => {
    const state = proxy({ user: { name: "John", age: 30 } });
    let renderCount = 0;

    const { result } = renderHook(() => {
      renderCount++;
      return useTrackedSnapshot(state, (s) => s.user);
    });

    expect(renderCount).toBe(1);
    expect(result.current).toEqual({ name: "John", age: 30 });

    // Mutate a property within the selected object.
    act(() => {
      state.user.age = 31;
    });

    // The hook must detect this change and trigger a re-render.
    await waitFor(() => {
  ...
  /**
   * Tests that a component re-renders when an element within an array
   * returned by the selector is mutated.
   */
  test("should re-render when an element in the returned array changes", async () => {
    const state = proxy({ items: [{ value: 10 }, { value: 20 }] });
    let renderCount = 0;

    const { result } = renderHook(() => {
      renderCount++;
      return useTrackedSnapshot(state, (s) => s.items);
    });

    expect(renderCount).toBe(1);
    expect(result.current).toEqual([{ value: 10 }, { value: 20 }]);

    // Mutate a nested property inside the selected array.
    act(() => {
      state.items[0].value = 11;
    });

    // The component should re-render with the updated array data.
    await waitFor(() => {
      expect(renderCount).toBe(2);
      expect(result.current[0].value).toBe(11);
    });
  });

  /**
   * Verifies that the hook remains robust by re-rendering correctly for both
   * internal mutations and structural replacements of the selected object.
   */
  test("should re-render on both internal mutation and structural replacement", async () => {
    const state = proxy({ user: { name: "John", age: 30 } });
    let renderCount = 0;
    ...
    // 3. Test internal mutation on the *new* object.
    act(() => {
      state.user.age = 26;
    });
    await waitFor(() => {
